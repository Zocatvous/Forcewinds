...
def gen_string(type_str):
	if type_str is "flurry":
		return f'{Fore.CYAN}{strike_count} xFlurry Attack(s){Style.RESET_ALL} w/ {Fore.GREEN}{weapon_acc+channel}{Style.RESET_ALL} Dice!'
	elif type_str is "strike":
		return f'{Fore.CYAN}{strike_count} Striking Attack(s){Style.RESET_ALL} w/ {Fore.GREEN}{weapon_acc+channel}{Style.RESET_ALL} Dice!'
...
def strike(weap_name,strike_count, weapon_acc, weapon_dam, opp_dv, opp_soak, heavy=False):
	...
	for i in range(0,strike_count):
		...
		if (successes+auto_succ_acc) > (opp_dv-i):
			damage_pool = weapon_dam+successes if (heavy==True) else (weapon_dam-opp_soak)+threshold
